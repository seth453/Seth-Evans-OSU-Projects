 ...
 *
 * Name: Seth Evans
 * Email: evansset@oregonstate.edu
 */
 ...
 */
struct ht* ht_create(){
    struct ht* ht = malloc(sizeof(struct ht)); //create hash table memory
    if (ht == NULL) { 
...
ht->buckets = malloc(ht->capacity * sizeof(struct dynarray*)); //create bucket memory
if (ht->buckets == NULL) {  
    printf("Memory allocation failed.\n");  
    free(ht); 
    return NULL;
}
//initialize each bucket as a dyn array
...
for (int i = 0; i < ht->capacity; ++i)
{
    dynarray_free(ht->buckets[i]); //free buckets
} 
//free buckets then table
free(ht->buckets); 
free(ht); 
...
int ht_hash_func(struct ht* ht, void* key, int (*convert)(void*)){
    int hash = convert(key) % ht->capacity; // calculate hash value
    return (hash < 0) ? hash + ht->capacity : hash; // make sure hash value is positive 
} 
...
void ht_insert(struct ht* ht, void* key, void* value, int (*convert)(void*)){
    int index = ht_hash_func(ht, key, convert); //insert key value pair
    struct dynarray* bucket = ht->buckets[index]; //get index from hash func
...
//create new entry for bucket, key doesnt exist
    struct ht_entry* new_entry = malloc(sizeof(struct ht_entry)); //memory for new entry 
    new_entry->key = key; 
    new_entry->value = value; 
    //insert new entry into the bucket and increase ht size
    dynarray_insert(bucket, new_entry); 
    ht->size++;
}
...
void* ht_lookup(struct ht* ht, void* key, int (*convert)(void*)) {
    // calculate index then get bucket there
...
void ht_remove(struct ht* ht, void* key, int (*convert)(void*)){
     ...
     for (int i = 0; i < dynarray_size(bucket); ++i)
     {
         ...
         if (entry != NULL && entry->key != NULL && convert(entry->key) == convert(key)) 
         {
             dynarray_remove(bucket, i); //remove bucket entry
             free(entry); 
